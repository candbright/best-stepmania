use serde::Serialize;
use sm_score::{TimingWindows, ScoreWeights, GradeThresholds};
...
fn life_delta_set(cfg: &LifeConfig) -> LifeDeltaSet {
    if cfg.life_type == sm_score::LifeType::Battery {
        return LifeDeltaSet { w1: 0.0, w2: 0.0, w3: 0.0, w4: 0.0, w5: 0.0, miss: 0.0, mine: 0.0 };
    }
    LifeDeltaSet {
        w1:   cfg.deltas.w1,
        w2:   cfg.deltas.w2,
...
pub fn get_scoring_config() -> ScoringConfig {
    ...
    ScoringConfig {
        timing_windows: TimingWindowsDto {
            w1: tw.w1, w2: tw.w2, w3: tw.w3, w4: tw.w4, w5: tw.w5,
            mine: tw.mine, hold: tw.hold, roll: tw.roll,
        },
